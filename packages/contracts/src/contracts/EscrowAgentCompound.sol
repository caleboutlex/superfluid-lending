...
//  11719484    17:41
    11719614    17:43:44
contract EscrowAgentCompound is EscrowToken, IERC777Recipient, IERC777Sender, SuperAppBase {
    ...
    function closeAgreement(uint256 id) external {
        ...
        if(amountToSend > 0 && daixBalance >=  amountToSend) {
            ...
            cdai.approve(address(cdai), cdaiBalance);
            cdai.redeem(cdaiBalance);

            dai.approve(address(acceptedToken), diff);
    ...
    function getHoldings(uint256 id) public view returns(uint256) {
        ...
        uint256 cdaiUnderlyingBalance = cdai.balanceOfUnderlying(address(this));
       
        uint256 contractBalance = daicontractBalance.add(daixcontractBalance).add(cdaiUnderlyingBalance);
    ...
    function rebalanceAll() internal {
        ...
        if(daixBalance < rebalanceAmount && cdaiBalance != 0) {
        ...
        } else if ( daixBalance < rebalanceAmount && cdaiBalance == 0 ) {
            // calc the diff in daix
            uint256 diff = rebalanceAmount.sub(daixBalance);
