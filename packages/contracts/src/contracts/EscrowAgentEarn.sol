...
contract EscrowAgentYearn is EscrowToken, IERC777Recipient, IERC777Sender, SuperAppBase {
    ...
    function closeAgreement(uint256 id) external {
        ...
        uint256 ydaiBalance = IERC20(address(ydai)).balanceOf(address(this));
        // to upgrade dai we need to withdraw ydai 
        IERC20(address(ydai)).approve(address(ydai), ydaiBalance);
        ydai.withdrawAll();
        ...
        if(amountToSend > 0 && daixBalance >=  amountToSend) {
            ...
            dai.approve(address(acceptedToken), daiBalance);
            acceptedToken.upgrade(daiBalance);
    ...
    function getHoldings(uint256 id) public view returns(uint256) {
        ...
        uint256 ydaicontractBalance = IERC20(address(ydai)).balanceOf(address(this));
        uint256 ydaiPricePerFullShare = ydai.getPricePerFullShare();
        uint256 convertedydaiBalance = ydaicontractBalance.mul(ydaiPricePerFullShare);

        uint256 contractBalance = daicontractBalance.add(daixcontractBalance).add(convertedydaiBalance);
    ...
    function rebalanceAll() internal {
        ...
        // if daix balance is smaller then the amount we need in the contract
        if(daixBalance < rebalanceAmount && ydaiBalance != 0) {
            // calc the diff in daix
            uint256 diff = rebalanceAmount.sub(daixBalance);

            // approve the contracts ydai to the ydai contract
            IERC20(address(ydai)).approve(address(ydai), ydaiBalance);

            // first me need to withdraw
            ydai.withdrawAll();

            // upgrade the amount we need
            upgradeDai(diff);

        } else if ( daixBalance < rebalanceAmount && ydaiBalance == 0 ) {
            // calc the diff in daix
            uint256 diff = rebalanceAmount.sub(daixBalance);
        ...
        // if daix balance is bigger then the amount we need in the contract
        } else if (daixBalance > rebalanceAmount) {

            // calc the diff to to get the amount of daix we need to downgrade 
        ...
        dai.approve(address(ydai), daiBalance);
        ydai.depositAll();
